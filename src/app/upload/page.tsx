...
export default function UploadPage() {
    ...
    const [multiResults, setMultiResults] = useState<Array<{ name: string; url: string }>>([]);
    ...
    async function handleUpload() {
        try {
            ...
            if (data?.results && Array.isArray(data.results)) {
                setMultiResults(data.results.map((r: any) => ({ name: r.name, url: r.url })));
            } else {
                                ...
                                <div className="flex-1 p-6 flex flex-col justify-center">
                                    <div className="space-y-6">
                                        <div className="space-y-3">
                                            <Label htmlFor="file" className="text-base font-medium">Chọn ảnh</Label>
                                            <input
                                                ref={fileInputRef}
                                                id="file"
                                                type="file"
                                            ...
                                            >
                                                <div className="text-center">
                                                    ...
                                                    <div className="font-semibold text-lg mb-2">
                                                        {(() => {
                                                            if (isAdmin && files.length > 1) return `${files.length} file đã chọn`;
                                                            return file ? file.name : "Click để chọn file ảnh";
                                                        })()}
                                                    </div>
                                    ...
                                    {multiResults.length > 0 ? (
                                            ...
                                            <div className="flex-1 bg-zinc-50 rounded-xl overflow-auto min-h-0 p-4">
                                                <div className="space-y-3">
                                                    {multiResults.map((item, idx) => (
                                                        <div key={idx} className="flex items-center justify-between gap-3 p-3 bg-white border rounded-lg">
                                                            <div className="min-w-0">
                                                                <div className="font-medium truncate">Tên ảnh: {item.name}</div>
                                                                <div className="text-sm text-zinc-600 truncate">URL: {typeof window !== 'undefined' ? (window.location.origin + item.url) : item.url}</div>
                                                            </div>
                                                            <Button size="sm" variant="outline" onClick={() => { navigator.clipboard.writeText((typeof window !== 'undefined' ? window.location.origin : '') + item.url); toast.success("Đã copy URL!"); }}>Copy URL</Button>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                        ...
                                        <div className="flex-1 flex items-center justify-center min-h-0">
                                            <div className="text-center text-zinc-500">
                                                ...
                                                <p className="text-sm">Chọn và upload ảnh để xem trước</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
