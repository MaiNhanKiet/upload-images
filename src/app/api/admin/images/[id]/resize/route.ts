...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        if ((!w && !h) || (w && w <= 0) || (h && h <= 0)) {
            return NextResponse.json({ error: "Width/Height không hợp lệ" }, { status: 400 });
        }
        ...
        for (const k of keys) {
            ...
            for (const s of arr) { try { const obj = JSON.parse(s); if (obj.id === imageId) { ownerEmail = k.replace(/^images:/, ''); meta = obj; break; } } catch { } }
            if (meta) break;
        }
        if (!meta) {
            return NextResponse.json({ error: "Ảnh không tồn tại" }, { status: 404 });
        }
        const relativeFromUrl = typeof meta.url === 'string' ? meta.url.replace(/^\//, '') : '';
        const filePath = path.join(process.cwd(), 'public', relativeFromUrl);

        const ext = path.extname(meta.fileName || '').toLowerCase();
        if (ext === '.svg') {
            return NextResponse.json({ error: "SVG là vector, không cần resize bằng raster" }, { status: 400 });
        }

        // Resize với sharp, giữ chất lượng tốt
        let pipeline = sharp(filePath).rotate();
        pipeline = pipeline.resize({ width: w || undefined, height: h || undefined, fit: 'inside', withoutEnlargement: true });

        if (ext === '.jpg' || ext === '.jpeg') {
            pipeline = pipeline.jpeg({ quality: 90, mozjpeg: true });
        } else if (ext === '.png') {
            pipeline = pipeline.png({ compressionLevel: 9, adaptiveFiltering: true, quality: 90 });
        }
        ...
        try {
            ...
            const idx = arr.findIndex((it: any) => it.id === meta.id);
