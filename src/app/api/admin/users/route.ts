...
export async function GET(request: Request) {
    try {
        ...
        const fullUsers: User[] = raw.map((s) => { try { return JSON.parse(s) as User; } catch { return null as any; } }).filter(Boolean);
        const users = fullUsers.map((u) => ({ id: u.id, email: u.email, role: u.role, createdAt: u.createdAt, storageMb: u.storageMb ?? 1024 }));
...
export async function POST(request: Request) {
    try {
        ...
        const user: User = {
            ...
            storageMb: typeof storageMb === 'number' && storageMb > 0 ? Math.floor(storageMb) : 1024
        };
