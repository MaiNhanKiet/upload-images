...
import { verifyToken, hashPassword, User } from "@/lib/auth";
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        let list: User[] = rawList.map((s) => { try { return JSON.parse(s) as User; } catch { return null as any; } }).filter(Boolean);
        let user: User | null = list.find((u) => u.id === userId) || null;
        if (!user && emailFromQuery) {
            user = list.find((u) => u.email === emailFromQuery) || null;
        }
        if (!user) {
            return NextResponse.json({ error: "User không tồn tại trong user_list" }, { status: 404 });
        }
        ...
        if (list.length > 0) await redisClient.rPush('user_list', list.map((u) => JSON.stringify(u)));
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        const list2: User[] = rawList2.map((s) => { try { return JSON.parse(s) as User; } catch { return null as any; } }).filter(Boolean);
        let found = list2.find((u) => u.id === userId) || (emailParam ? list2.find((u) => u.email === emailParam) : undefined);
        if (!found) {
            return NextResponse.json({ error: "User không tồn tại trong user_list" }, { status: 404 });
        }
        ...
        await redisClient.del('user_list');
        if (next.length > 0) await redisClient.rPush('user_list', next.map((u) => JSON.stringify(u)));
        ...
        try {
            ...
            for (const s of arr) {
                try {
                    const image = JSON.parse(s);
                    if (image && typeof image.url === 'string') {
                        const relativeFromUrl = image.url.replace(/^\//, '');
                        const filePathFromUrl = path.join(process.cwd(), 'public', relativeFromUrl);
                        try { await unlink(filePathFromUrl); } catch { }
                    }
                } catch { }
            }
