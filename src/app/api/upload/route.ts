...
export async function POST(request: Request) {
  try {
    ...
    if (files.length > 1 && decoded.role !== "admin") {
      return NextResponse.json(
        { error: "Chỉ admin được phép upload nhiều ảnh" },
        { status: 403 }
      );
    }
    ...
    try {
      ...
      for (const s of list) {
        try {
          ...
          if (u.email === userEmail && typeof u.storageMb === "number") {
            storageMb = Math.max(1, Math.floor(u.storageMb));
            break;
          }
        } catch {}
      }
    ...
    try {
      ...
      for (const item of list) {
        try {
          ...
          if (typeof obj.size === "number") usedBytes += obj.size;
        } catch {}
      }
    } catch {}
    ...
    if (newTotal > quotaBytes) {
      ...
      return NextResponse.json(
        {
          error: `Bạn đã hết dung lượng. Còn lại ${Math.round(
            remaining / (1024 * 1024)
          )} MB`,
          code: "QUOTA_EXCEEDED",
        },
    ...
    // Store files outside of public to avoid exposing raw filesystem — use storage/uploads-images
    const uploadsDir = path.join(process.cwd(), "storage", "uploads-images");
    ...
    for (const file of files) {
      ...
      if (!allowedExt.has(ext) || !allowedMime.has(mime)) {
        return NextResponse.json(
          { error: `Định dạng không hỗ trợ: ${file.name}` },
          { status: 400 }
        );
      }
      ...
      // Tạo URL trả về (trực tiếp, không dùng basePath)
      // Return a public API URL that will serve the file from storage
      const url = `/api/uploads-images/${uuidFileName}`;
